...
export default function PaymentDetailPage() {
  ...
  const progressPercentage = paymentTypeAmount > 0 ? (totalPaid / paymentTypeAmount) * 100 : 0;
  ...
  const handleSubmitPayment = async (e: React.FormEvent) => {
    ...
    if (paymentOption === 'partial' && amount > remainingAmount) {
      toast.error(`Amount cannot exceed remaining balance of ${formatCurrency(remainingAmount)}`);
      return;
    }
    ...
    try {
      ...
      const students = selectedStudents.length > 0 
        ? [
            { id: user.id, full_name: user.full_name, reg_number: user.reg_number, level: user.level },
            ...selectedStudents
          ]
        : [{ id: user.id, full_name: user.full_name, reg_number: user.reg_number, level: user.level }];
      ...
      for (const student of students) {
        ...
        if (selectedMethod !== 'cash' && receiptFile) {
          ...
          const fileName = `${student.id}_${Date.now()}_${Math.random().toString(36).substring(7)}.${fileExt}`;
        ...
        const finalTransactionRef = selectedMethod === 'cash' 
  ...
  const handleResubmitRejected = (rejectedPayment: ExistingPayment) => {
    ...
    setNotes(`Resubmission of rejected payment. Previous reason: ${rejectedPayment.rejection_reason}`);
    setStep('method');
  };
  ...
  if (!paymentType) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: gradients.darkBackground }}>
        <GlassCard>
          <div className="text-center p-8">
            <AlertCircle className="w-16 h-16 mx-auto mb-4" style={{ color: colors.statusUnpaid }} />
            <h2 className="text-2xl font-bold text-white mb-2">Payment Type Not Found</h2>
            <button
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 relative z-10 w-full">
                ...
                {existingPayments.map((payment) => (
                  <div
                    ...
                    style={{
                      ...
                      borderColor: 
                        payment.status === 'approved' ? `${colors.statusPaid}40` :
                        payment.status === 'rejected' ? `${colors.statusUnpaid}40` :
                        `${colors.warning}40`
                    }}
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          {payment.status === 'approved' && <CheckCircle className="w-5 h-5" style={{ color: colors.statusPaid }} />}
                          {payment.status === 'rejected' && <XCircle className="w-5 h-5" style={{ color: colors.statusUnpaid }} />}
                          {payment.status === 'pending' && <Clock className="w-5 h-5" style={{ color: colors.warning }} />}
                          <span className="font-bold text-white">{formatCurrency(payment.amount)}</span>
                          <span
                            className="px-2 py-1 rounded-full text-xs font-medium"
                            style={{
                              background: 
                                payment.status === 'approved' ? `${colors.statusPaid}20` :
                                payment.status === 'rejected' ? `${colors.statusUnpaid}20` :
                                `${colors.warning}20`,
                              color: 
                                payment.status === 'approved' ? colors.statusPaid :
                                payment.status === 'rejected' ? colors.statusUnpaid :
                                colors.warning
                            }}
                          >
                        ...
                        <p className="text-sm" style={{ color: colors.textSecondary }}>
                          Submitted: {formatDate(payment.submitted_at, 'long')}
                        </p>
                        {payment.rejection_reason && (
                          <div className="mt-2 p-2 rounded" style={{ background: `${colors.statusUnpaid}10` }}>
                            <p className="text-sm font-medium" style={{ color: colors.statusUnpaid }}>
          ...
          >
            <GlassCard>
              <div className="flex items-start gap-4 p-4" style={{ background: `${colors.warning}10` }}>
                ...
                <div>
                  <p className="font-bold text-white mb-1">Payment Pending Review</p>
                  <p className="text-sm" style={{ color: colors.textSecondary }}>
                    You have a payment awaiting approval. Please wait for the admin to review your submission before making another payment.
                  </p>
                </div>
              </div>
            </GlassCard>
        ...
        {!isFullyPaid && !hasPendingPayment && (
          <>
            ...
            {step === 'upload' && (selectedMethod === 'bank_transfer' || selectedMethod === 'pos') && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="space-y-6"
              >
                ...
                {paymentType.allow_partial && remainingAmount > 0 && (
                  <GlassCard>
                    <h3 className="text-lg font-bold text-white mb-4">Payment Amount</h3>
                    
                    {/* Info Box */}
                    <div className="mb-4 p-3 rounded-lg" style={{ background: `${colors.accentMint}15` }}>
                      <p className="text-sm" style={{ color: colors.accentMint }}>
                        💡 This payment type allows partial payments. You can pay the full amount or any amount you're comfortable with.
                      </p>
                    </div>
                    ...
                    <div className="space-y-3">
                      <button
                        ...
                        style={{
                          borderColor: paymentOption === 'full' ? colors.primary : 'rgba(255, 255, 255, 0.1)',
                      ...
                      <button
                        ...
                        className="w-full p-4 rounded-lg border-2 transition-all text-left"
                        style={{
                          borderColor: paymentOption === 'partial' ? colors.primary : 'rgba(255, 255, 255, 0.1)',
                      ...
                      {paymentOption === 'partial' && (
                        ...
                        >
                          <label className="block text-sm font-medium mb-2 text-white">
                            Enter Amount (Max: {formatCurrency(Math.max(0, remainingAmount))})
                          </label>
                          <input
                            type="number"
                            value={amount || ''}
                          ...
                          <p className="text-xs mt-1" style={{ color: colors.textSecondary }}>
                            You'll be able to pay the remaining {formatCurrency(Math.max(0, remainingAmount - amount))} later
                          </p>
                        </motion.div>
                      )}
                    </div>
                  ...
                  <GlassCard>
                    ...
                    <div className="space-y-4">
                      ...
                      {selectedStudents.length > 0 && (
                        <div className="p-4 rounded-lg" style={{ background: `${colors.primary}10`, border: `1px solid ${colors.primary}40` }}>
                          <p className="font-bold text-white mb-2">
                            Paying for {selectedStudents.length + 1} student(s):
                          </p>
                          <ul className="text-sm space-y-1" style={{ color: colors.textSecondary }}>
                            <li>• You</li>
                            {selectedStudents.map(s => (
                              <li key={s.id}>• {s.full_name} ({s.reg_number})</li>
                            ))}
                          </ul>
                          <p className="text-lg font-bold mt-3" style={{ color: colors.primary }}>
                            Total: {formatCurrency((selectedStudents.length + 1) * (paymentOption === 'full' ? remainingAmount : amount))}
                          </p>
                        </div>
                      )}
                      ...
                      >
                        {submitting ? (
                          ...
                          `Submit Payment${selectedStudents.length > 0 ? ` for ${selectedStudents.length + 1} Student(s)` : ''}`
                        )}
                      </button>
                    </div>
                  </GlassCard>
